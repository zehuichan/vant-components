  ...
  render() {
      ...
      <div class="v-multiple-picker__content">
        {
          this.formattedColumns.map((item, columnIndex) => (
            <div class="v-multiple-picker__group">
              {item.title ? <div class="v-multiple-picker__title">{item.title}</div> : null}
              {renderOptions(item)}
            </div>
          ))
        }
    ...
    const renderFooter = () => (
      <div class="v-multiple-picker__footer">
        <van-button type="primary" size="small" onClick={this.onConfirm}>确定</van-button>
      </div>
    )

    return (
      <van-action-sheet value={true} title={this.title} round={false} get-container="body">
        {renderColumns()}
        {renderFooter()}
      </van-action-sheet>
    )
